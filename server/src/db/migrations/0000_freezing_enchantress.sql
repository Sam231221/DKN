...
CREATE TYPE "public"."user_role" AS ENUM('client', 'employee', 'consultant', 'knowledge_champion', 'administrator', 'executive_leadership', 'knowledge_council_member');--> statement-breakpoint
...
ALTER TABLE "knowledge_assets" ADD CONSTRAINT "knowledge_assets_project_id_projects_id_fk" FOREIGN KEY ("project_id") REFERENCES "public"."projects"("id") ON DELETE no action ON UPDATE no action;--> statement-breakpoint
...
ALTER TABLE "training_sessions" ADD CONSTRAINT "training_sessions_knowledge_champion_id_users_id_fk" FOREIGN KEY ("knowledge_champion_id") REFERENCES "public"."users"("id") ON DELETE no action ON UPDATE no action;